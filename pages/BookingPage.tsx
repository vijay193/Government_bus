...
import type { Schedule, SeatBookingInfo, BusLocation } from '../types';
...
export const BookingPage: React.FC = () => {
  ...
  const { fare: fareFromState, userOrigin, userDestination } = (location.state as { fare: number, userOrigin: string, userDestination: string }) || {};
  ...
  const [selectedOrigin, setSelectedOrigin] = useState(userOrigin || '');
  const [selectedDestination, setSelectedDestination] = useState(userDestination || '');
  ...
  useEffect(() => {
    ...
    const fetchInitialData = async () => {
      ...
      setError(null);
      try {
        const [scheduleData, busLocationData] = await Promise.all([
          api.getScheduleById(scheduleId),
          api.trackBus(scheduleId).catch(() => null)
        ]);
        ...
        if (scheduleData) {
            ...
            const stops = scheduleData.fullRouteStops;
            if (!stops || stops.length < 2) {
                setError("Route information is incomplete for this schedule.");
                setSchedule(null);
                return;
            }

            const allOriginOptions = stops.slice(0, -1).map((stop, index) => {
                let isDeparted = false;
                if (busLocationData) {
                    if (index < busLocationData.currentStopIndex) {
                        isDeparted = true;
                    } else if (index === busLocationData.currentStopIndex && !busLocationData.isAtStop) {
                        isDeparted = true;
                    }
                }
                return { ...stop, isDisabled: isDeparted };
            });

            const initialOriginName = userOrigin || scheduleData.origin || '';
            const initialOriginIsDeparted = allOriginOptions.find(o => o.name === initialOriginName)?.isDisabled ?? false;

            let finalOrigin = initialOriginName;
            let finalDestination = userDestination || scheduleData.destination || '';

            if (initialOriginIsDeparted) {
                const nextAvailableOrigin = allOriginOptions.find(o => !o.isDisabled);
                
                if (nextAvailableOrigin) {
                    finalOrigin = nextAvailableOrigin.name;
                    const newOriginIndex = stops.findIndex(s => s.name === finalOrigin);
                    if (newOriginIndex !== -1 && stops[newOriginIndex + 1]) {
                        finalDestination = stops[newOriginIndex + 1].name;
                    } else {
                        finalDestination = stops[stops.length - 1].name;
                    }
                } else {
                    setError("This bus has already departed from all available boarding points. Booking is no longer possible.");
                    setSchedule(null);
                    return;
                }
            }
            
            setSelectedOrigin(finalOrigin);
  ...
  useEffect(() => {
    if (!scheduleId || !selectedOrigin || !selectedDestination || !schedule) {
        return;
    }

    const fetchBookedSeatsForSegment = async () => {
        setIsSeatLayoutLoading(true);
        setError(null);
        setSelectedSeats([]);
        setSeatDetails({});
        try {
            const bookedSeatsData = await api.getBookedSeatsForSchedule(scheduleId, selectedOrigin, selectedDestination);
            setBookedSeats(bookedSeatsData);
        } catch (err) {
            setError("Could not update seat availability for the selected route. Please try again.");
            setBookedSeats([]);
        } finally {
    ...
    fetchBookedSeatsForSegment();
  }, [scheduleId, selectedOrigin, selectedDestination, schedule]);
  ...
  const originOptions = useMemo(() => {
    if (!schedule?.fullRouteStops) return [];
    return schedule.fullRouteStops.slice(0, -1).map((stop, index) => {
        let isDeparted = false;
        if (busLocation) {
            if (index < busLocation.currentStopIndex) {
                isDeparted = true;
            } else if (index === busLocation.currentStopIndex && !busLocation.isAtStop) {
                isDeparted = true;
            }
        }
        return { ...stop, isDisabled: isDeparted };
  ...
  const destinationOptions = useMemo(() => {
    if (!schedule?.fullRouteStops || !selectedOrigin) return [];
    const originIndex = schedule.fullRouteStops.findIndex(s => s.name === selectedOrigin);
    if (originIndex === -1) return [];
    return schedule.fullRouteStops.slice(originIndex + 1);
  }, [schedule, selectedOrigin]);

  const handleOriginChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
      const newOrigin = e.target.value;
      setSelectedOrigin(newOrigin);

      const originIndex = schedule?.fullRouteStops.findIndex(s => s.name === newOrigin) ?? -1;
      const destinationIndex = schedule?.fullRouteStops.findIndex(s => s.name === selectedDestination) ?? -1;

      if (originIndex !== -1 && originIndex >= destinationIndex) {
          const nextStop = schedule?.fullRouteStops[originIndex + 1];
          if (nextStop) {
              setSelectedDestination(nextStop.name);
          }
      }
  };
  ...
  const pricePerSeat = useMemo(() => {
    if (!schedule?.fullRouteStops || !selectedOrigin || !selectedDestination) {
        return fareFromState ?? schedule?.fare ?? SEAT_PRICE;
    }
    const originStop = schedule.fullRouteStops.find(s => s.name === selectedOrigin);
    const destStop = schedule.fullRouteStops.find(s => s.name === selectedDestination);
    if (originStop && destStop && destStop.fare > originStop.fare) {
        return destStop.fare - originStop.fare;
    }
    return fareFromState ?? schedule?.fare ?? SEAT_PRICE;
  }, [schedule, selectedOrigin, selectedDestination, fareFromState]);
  ...
  const generatePdfReceipt = async (bookingId: string) => {
    ...
    doc.text(`Route: ${selectedOrigin} to ${selectedDestination}`, 20, 45);
    doc.text(`Bus: ${schedule?.busName} (${schedule?.id})`, 20, 55);
    doc.text(`Booking Date: ${new Date().toLocaleString()}`, 20, 65);
    doc.text(`Departure from ${selectedOrigin}: ${departureTime}`, 20, 75);
    doc.text(`Arrival at ${selectedDestination}: ${arrivalTime}`, 20, 82);
    ...
    const qrPayload = {
        ...
        timings: {
            departure: `Departure from ${selectedOrigin}: ${departureTime}`,
            arrival: `Arrival at ${selectedDestination}: ${arrivalTime}`,
        },
        seats: selectedSeats,
        fare: {
    ...
    <div className="container booking-page">
      ...
      <div className="booking-page__layout">
        <div className={`booking-page__seat-area ${isSeatLayoutLoading ? 'booking-page__seat-area--loading' : ''}`}>
            {isSeatLayoutLoading && <div className="booking-page__seat-loader"><div className="home-page__spinner"></div></div>}
            <SeatLayout 
        ...
        <div className="booking-page__summary-area">
          <Card className="booking-page__summary-card">
            ...
            <div className="booking-page__route-selector">
                <div className="input-wrapper">
                    <label htmlFor="origin-select" className="input-label">From</label>
                    <select id="origin-select" value={selectedOrigin} onChange={handleOriginChange} className="input-field" disabled={isLoading || !schedule}>
                        {originOptions.map(stop => (
                            <option key={stop.name} value={stop.name} disabled={stop.isDisabled}>
                                {stop.name}{stop.isDisabled ? ' (Departed)' : ''}
                            </option>
                        ))}
                    </select>
                </div>
                <ArrowRight className="booking-page__route-arrow" size={24} />
                <div className="input-wrapper">
                    <label htmlFor="destination-select" className="input-label">To</label>
                    <select id="destination-select" value={selectedDestination} onChange={handleDestinationChange} className="input-field" disabled={isLoading || !schedule}>
